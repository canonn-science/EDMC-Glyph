...
def plugin_app(parent):
    ...
    this.status=tk.Label(this.frame, text="Waiting for scan", foreground="green")
    ...
    this.inners=itertools.cycle(list(range(1,11)))
...
def display():
    ...
    id=f"{this.outer_value}-{this.inner_value}"
    gif=f"{id}.gif"
    this.glyph_image = tk.PhotoImage(file=os.path.join(this.plugin_dir, "images", gif))
    this.ship_image = tk.PhotoImage(file=os.path.join(this.plugin_dir, "images", f"{this.interceptor}.gif"))
    this.glyph["image"]=this.glyph_image
    this.ship["image"]=this.ship_image

    this.glyph_id["text"]=id
